...
function navigateToDetails(media: Media) {
  mediaStore.trailers = []
  mediaStore.mediaDetails = media
  router.push({
    name: 'mediaDetails',
    params: {
      id: props.media.id,
...
<template>
  <div class=" items-center p-4 rounded-lg transition-transform transform hover:-translate-y-1 ">
    <div class="h-8 z-10 absolute top-1 lg:right-0 right-4 bg-neutral-900 bg-opacity-75 text-yellow-300 text-sm lg:text-base font-semibold rounded-full px-2 py-1 flex items-center">
      <span class="mr-1">{{ Math.floor(props.media.vote_average) }}</span>
    ...
    <img
      :src="getImage(props.media.poster_path)"
      :alt="props.media.name"
      class="z-5 cursor-pointer hover:shadow-lg rounded-sm h-auto mb-3 transition-opacity duration-200 hover:opacity-90"
      @click="navigateToDetails(props.media)"
    >
